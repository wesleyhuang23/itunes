  ...
  //You can return the http request or you can make your own promise in order to manipulate the data before you resolve it.
    ...
    this.getSongData = function(artist){
      ...
      return $http.jsonp('https://itunes.apple.com/search?term=' + artist + '&callback=JSON_CALLBACK').then(function(response){
        ...
        var musicData = response.data.results;
        console.log(musicData[0].artistName);
        console.log(musicData.length);
        this.compressed = [];
        this.obj = {};
        for(var i = 0; i < musicData.length; i++){
          obj.AlbumArt = musicData[i].artworkUrl100;
          obj.Artist = musicData[i].artistName;
          obj.Collection = musicData[i].collectionName;
          obj.CollectionPrice = musicData[i].collectionPrice;
          obj.Play = musicData[i].previewUrl;
          compressed.push(obj);
        }
